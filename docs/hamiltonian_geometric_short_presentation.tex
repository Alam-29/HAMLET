...
\begin{frame}{Limitations}
  \begin{itemize}
    ...
    \item \textbf{Statistics}: most results are single-seed point estimates; only the
      modern-optimizer sweep has 10-seed variance.
    \item \textbf{Tuning}: learning rates from modest grids, not exhaustive search.
    \item \textbf{Comparability}: the modern-optimizer run uses a diagonal reduction, not the full
      curvature-memory-entropy architecture.
  \end{itemize}
\end{frame}
