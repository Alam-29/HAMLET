...
% ---------- 9. Multi-seed reality check ----------
\begin{frame}{Multi-Seed Reality Check --- Do the Single-Seed Wins Hold Up?}
  \scriptsize
  \begin{tabular}{@{}p{0.28\linewidth}rrrr@{}}
    ...
    DeepOBS-style (val.\ loss) & 10 & $0.0580\pm0.039$ & $0.0551\pm0.025$ & $0.689$ \\
    WikiText-2 LLM (val.\ loss) & 3 & $7.368\pm0.015$ & $7.237\pm0.006$ & $\mathbf{0.003}$ \\
    \bottomrule
  \end{tabular}
  \vspace{0.7em}
  \begin{itemize}
    \item \textbf{DeepOBS-style is the most consequential reversal here}: the single-seed table
      showed a $2\times$ win; across 10 seeds HG wins only 5/10 --- a coin flip, $p=0.689$.
      That single-seed entry was a favorable draw of the seed, not a reliable advantage.
    \item MNIST: HG wins 7/10 seeds but not significantly ($p=0.688$). AlgoPerf-style: borderline
      favorable (8/10, $p=0.067$).
    \item \textbf{WikiText-2 LLM: the opposite correction.} AdaFactor beats HG in all 3 seeds, and
      the gap is significant ($p=0.003$) despite only 3 seeds --- the earlier ``2nd best'' framing
      understated how consistent AdaFactor's edge actually is.
    \item PINN multi-seed was attempted but not completed: its finite-difference metric makes a
      single seed far more expensive than the other three combined --- an honest, costed gap,
      not a result.
  \end{itemize}
\end{frame}

% ---------- 10. Modern optimizer comparison (3 graphs, one slide) ----------
\trifigsnote
...
% ---------- 11. Modern optimizer benchmark table ----------
...
% ---------- 12. Extended ablation study: evidence (3 graphs, one slide) ----------
...
% ---------- 13. Extended ablation study: key numbers ----------
...
\begin{frame}{Limitations}
  \begin{itemize}
    ...
    \item \textbf{Statistics}: multi-seed variance now covers the modern-optimizer sweep (10
      seeds), the classical/LLM reality check (10/10/10/3 seeds), and the extended ablation
      (40/10/10 seeds) --- and it is not uniformly favorable: the DeepOBS single-seed win does
      not replicate, and the LLM loss to AdaFactor is significant, not a near-tie.
...
\begin{frame}{Summary}
  \begin{itemize}
    ...
    \item The multi-seed reality check cuts both ways too --- it takes away a single-seed win
      (DeepOBS) as much as it takes away worry about a near-tie (LLM loss to AdaFactor is real).
  \end{itemize}
  \vfill
  \centering
