...
\title{\color{hgpurple}\textbf{Hamiltonian-Geometric Optimization}\\[2pt]
  \large Speaker Script and Anticipated Q\&A (15-Slide Deck)}
...
so the full talk runs about 9--11 minutes). Part~\ref{sec:qa} is a set of hard, likely audience
...
not a scaling claim. And I mean that literally --- the next slide is exactly that check.''

\subsection*{Slide 9 --- Multi-Seed Reality Check}
``We went back and re-ran three of those four workloads across 10 seeds each, and the LLM run
across 3, using each benchmark's own script, not a re-implementation. I want to walk through this
slide slowly because it's the most important honesty check in the whole talk. MNIST: the win
doesn't hold up statistically, $p=0.688$. AlgoPerf-style: borderline favorable, $p=0.067$. DeepOBS
is the big one --- the single-seed table showed us winning by more than two-to-one; across 10
seeds we win exactly half of them. That was a lucky seed, not a real effect, and I'd rather tell
you that than let the earlier table stand unchallenged. And then the LLM row flips the other way:
we said `2nd best' before, like it was basically a tie --- it isn't. AdaFactor beats us in all
three seeds and the gap is statistically significant. Last point: we tried to add PINN to this
seed check too and couldn't finish it in the compute budget we had --- its geometric force needs
finite differences over 64 dimensions every step, and that alone is why it's not in this table.
That's a cost limitation, not a result, and I'm flagging it as exactly that.''

\subsection*{Slide 10 --- Modern Optimizer Comparison}
...
\subsection*{Slide 11 --- Benchmark Results Table (Modern Optimizers)}
...
\subsection*{Slide 12 --- Extended Ablation Study: Design \& Evidence}
...
\subsection*{Slide 13 --- Extended Ablation Study: Key Numbers}
...
\subsection*{Slide 14 --- Limitations}
``Four honest caveats. Scale: everything here is small; a full n-by-n metric is a mechanism
demonstration, not something we're claiming replaces Adam in production. Statistics: we now have
real multi-seed variance from three places --- the modern-optimizer sweep, the classical/LLM
reality check, and the extended ablation --- and it's not uniformly kind to us: DeepOBS's win
evaporates and the LLM loss is real, not noise. Tuning: learning rates come from modest grids,
not exhaustive search. And comparability: both the modern-optimizer run and the CUDA ablation use
a diagonal reduction, not the full curvature-memory-entropy architecture.''

\subsection*{Slide 15 --- Summary and Thank You}
...
optimizers, we're in the leading group, not decisively ahead of Adam or Muon. And every rigor pass
we added sharpens that picture rather than softening it: a real, honest positive on the spectral
term under a true Hessian metric; a real, honest negative on quantum control, GPU-scale diagonal
reduction, and now a WikiText-2 loss to AdaFactor that a 3-seed check shows is not noise; and one
single-seed win, on DeepOBS, that the multi-seed check took back. Happy to take questions,
especially on the validation methodology, the ablation results, or where we'd want to push this
next.''
...
\item \textbf{You added a multi-seed check and it took away a win. Why would you do that to your own paper?}\\
Because a single favorable seed on DeepOBS-style ($0.0203$ vs.\ Adam's $0.0449$) was never a
claim we could stand behind once asked ``does that replicate?'' --- and the honest answer, once
we ran it, is no: 10 paired seeds give HG a win rate of 5/10 and $p=0.689$, indistinguishable from
chance. We'd rather retract that specific claim ourselves, in our own report, than have someone
else's replication attempt do it for us. The same check also went the other way: the WikiText-2
loss to AdaFactor, which the single-seed table softened to ``2nd best,'' turns out to be
consistent and significant across seeds ($p=0.0027$) --- so this wasn't a one-directional
correction, it's just what happens when you actually check.

\item \textbf{Is there any real theorem in this paper, or is it all empirical?}\\
One, stated and proved for exactly the setting where it's provable in closed form: for a quadratic
loss with the metric set to the exact (constant) Hessian, every correction term vanishes
identically, and the resulting momentum recursion is provably stable and linearly convergent for
any $0<\eta<\sqrt2$, $0\le\beta<1$, with a rate depending only on $(\eta,\beta)$ --- never on the
Hessian's condition number. That's a real, checkable proof, not a citation to someone else's
convergence theorem. It's deliberately narrow: it says nothing about the correction terms' effect
when they're active on a non-quadratic loss, or about the regularized, $\theta$-dependent metric
actually used in the benchmarks --- those remain empirical claims, backed by the ablations, not
by this theorem.
