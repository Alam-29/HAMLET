...
so the full talk runs about 7--9 minutes). Part~\ref{sec:qa} is a set of hard, likely audience
...
\subsection*{Slide 11 --- Limitations}
``Four honest caveats. Scale: everything here is small; a full n-by-n metric is a mechanism
demonstration, not something we're claiming replaces Adam in production. Statistics: most results
are single-seed; the 10-seed sweep is the only place we have real variance data. Tuning: learning
rates come from modest grids, not exhaustive search. And comparability: the modern-optimizer
benchmark uses a reduced, diagonal version of our method, so that comparison specifically isn't
testing the full framework.''

\subsection*{Slide 12 --- Summary and Thank You}
``Three takeaways. The formulation holds together once you're explicit about the spectral-force
sign and the memory-metric construction. The empirical evidence is strongest exactly where the
theory predicts it should be --- where there's real, exploitable curvature. And against modern
optimizers, we're in the leading group, not decisively ahead of Adam or Muon. Happy to take
questions, especially on the validation methodology or where we'd want to push this next.''
